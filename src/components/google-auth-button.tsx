...
export function GoogleAuthButton({ mode }: GoogleAuthButtonProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(`Gagal memulai autentikasi Google: ${error.message}`);
        setIsLoading(false);
      }
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="w-full flex items-center justify-center gap-3 rounded-2xl border-2 border-neutral-200 bg-white px-4 py-2.5 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50 hover:border-neutral-300 disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {isLoading ? (
        <>
        ...
        <>
          <GoogleIcon />
          <span>{mode === "login" ? "Masuk" : "Daftar"} dengan Google</span>
        </>
      )}
    </button>
  );
